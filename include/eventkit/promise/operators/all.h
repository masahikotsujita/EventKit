...
void addCoreAsHandlerToPromisesAt(const std::shared_ptr<Cr>& pCore, LastPr&& lastPr) {
...
void addCoreAsHandlerToPromisesAt(const std::shared_ptr<Cr>& pCore, PrAtIndex&& promiseAtIndex, RestPrs&& ...restPromises) {
...
void addCoreAsHandlerToPromises(const std::shared_ptr<Cr>& pCore, Prs&& ...promises) {
...
auto all(Prs&& ...promises) {
    ...
    auto pCore = std::make_shared<ek::promise::detail::AllTransformationCore<std::decay_t<Prs>...>>();
